...
NOTE_USER = "keirin_border2"   # noteのID。X告知文の記事URLに使う
PRICE = 100          # 出走メンバー部分の価格。0にすると全文無料
TAGS = ["競輪", "勝負駆け", "代謝ボーダー"]   # 先頭の # は自動で付く
...
def main() -> int:
    ...
    try:
        # X告知文。{url} は公開時に記事URLへ置き換わる。
        # 280字制限: 本文約60字 + URL23字 + タグ ≒ 100字で余裕あり
        promo = (f"【{target.month}/{target.day}】競輪 勝負駆け・代謝危機の"
                 f"出走まとめを公開しました🚴\n"
                 f"この投稿をリポストすると出走メンバーが無料で読めます\n"
                 f"{{url}}\n"
                 f"#競輪 #勝負駆け #代謝ボーダー #note")
