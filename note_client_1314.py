...
class NoteClient:
    ...
    def publish(self, note_id: int, title: str, text: str,
        ...
        # SNSプロモーションは公開しただけでは status=reserved のままで、
        # このAPIを叩いて初めてnoteがXへ投稿し available になる。
        # (ブラウザの「投稿する内容を保存」が呼んでいるのと同じもの)
        if campaigns and note_key:
            self.activate_twitter_campaign(note_key)
    ...
    def activate_twitter_campaign(self, note_key: str) -> None:
        """リポスト割引キャンペーンを有効化する(noteがXへ投稿する)。

        失敗しても記事は公開済みなので、警告を出すだけで止めない。
        """
        try:
            r = self.session.post(
                f"{NOTE_BASE}/v3/discount_campaigns/twitter/post_status",
                json={"note_key": note_key})
            if r.ok:
                print("[ok] Xに投稿し、リポスト割引を有効化しました")
            else:
                print(f"[warn] リポスト割引の有効化に失敗: "
                      f"{r.status_code} {r.text[:200]}")
        except Exception as e:
            print(f"[warn] リポスト割引の有効化で例外: {e}")

    def create_and_publish(self, title: str, text: str,
                           hashtags=None, price: int = 0,
                           free_text: str | None = None, promo_text: str = "",
                           is_refund: bool = False, note_user: str = "",
        ...
        # X告知文の {url} を記事URLに置き換える。
        # noteの定型文と違い、自分で文面を渡すとURLもタグも付かないため、
        # ここで埋め込んでおかないとリンク無しの投稿になる。
        if promo_text and "{url}" in promo_text:
            url = (f"https://note.com/{note_user}/n/{note_key}"
                   if note_user and note_key else "")
            promo_text = promo_text.replace("{url}", url).strip()
